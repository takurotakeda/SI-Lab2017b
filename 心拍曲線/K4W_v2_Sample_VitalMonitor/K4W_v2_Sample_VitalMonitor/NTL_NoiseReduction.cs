...
namespace NTL.KinectLib
{
    ...
    public class NoiseReduction
    {
        ...
        {
            public float A; //【放物線係数】
            public float B; //【放物線係数】
            public float C; //【放物線係数】
            public float D; //【放物線の軸】d=-b/(2*a)
        }
        ...
        //【Estimation】
        //*************************************************************
        public void Estimation(float data)
        {
            FIFO[wPtr] = data;
            rPtr = wPtr;
            //【書込みポインタの更新】
        ...
        public float A { get { if (FIFOfull) { return a; } else { return 0.0f; } } }
        public float B { get { if (FIFOfull) { return b; } else { return 0.0f; } } }
        public float C { get { if (FIFOfull) { return c; } else { return 0.0f; } } }
        public float D { get { if (FIFOfull) { return d; } else { return -1000.0f; } } }
        ...
        private void ParabolaEstimation()
        {
            int j;
            float t, t2, t4, e;
            float Sa, Sb, Sc; // (working parameter) Sa=Σt^4*(Sj-Oj), Sb=Σt^2*(Sj-Oj), Sc=Σ(Sj-Oj)
            //【Estimate Xc】
            Sa = 0.0f;
            for (j = 0; j < nSample; j++)
            {
